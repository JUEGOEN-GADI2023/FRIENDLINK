...
<?php include('session.php'); ?>    
	...
	<style>
  .photo {
    max-width: 100%; /* Hace que todas las imágenes tengan un ancho máximo del 100% del contenedor */
...
<br><button type="submit" name="submit" class="btn btn-success"><i class="icon-upload"></i> Subir Foto</button>
  ...
  while ($row = $query->fetch()) {
    ...
    <div class="col-md-4 col-sm-4 text-center photo-container">
      <img class="photo" src="<?php echo $row['location']; ?>" alt="Imagen">
      <hr>
    ...
    // Cierra la fila después de mostrar 3 imágenes
    if ($count % 3 == 0) {
