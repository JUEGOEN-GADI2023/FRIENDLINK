<?php
session_start();
if (isset($_SESSION['unique_id'])) {
  header("location: users.php");
}
?>

<?php include_once "header.php"; ?>

<body>
  <div class="wrapper">
    <section class="form login">
        <center>
        <header>
           
            <h1>Chat Live</h1>
            <img  width="90px" src="https://images.vexels.com/media/users/3/214671/isolated/preview/7dec94c3cb64f4119d51e69ab5b12011-icono-de-trazo-de-burbujas-de-chat.png" alt="Logo del Chat Live"  >
        </header>
     
      </center>
      <form action="#" method="POST" enctype="multipart/form-data" autocomplete="off">
          <center>
          <h1>iniciar sección</h1> <br>
           </center>
        <div class="error-text"></div>
        <div class="field input">
          <label>Dirección de Correo Electrónico</label>
          <input type="text" name="email" placeholder="Ingresa tu Correo Registrado" required>
        </div>
        <div class="field input">
          <label>Contraseña</label>
          <input type="password" name="password" placeholder="Ingresa tu Contraseña" required>
          <i class="fas fa-eye"></i>
        </div>
        <div class="field button">
            <input type="submit" name="submit" value="Confirmar Datos">
            <button class="op"><a href="index.php">Iniciar seccion</a></button>
        </div>
      </form>
       
        
      <div class="link">Aún no te has Unido? <a href="index.php">Regístrate acá</a></div>
    </section>
  </div>

  <script src="javascript/pass-show-hide.js"></script>
  <script src="javascript/login.js"></script>

</body>

</html>